...
import {
  ExperimentalSideQuestsPanel,
  SHOW_TELL_MODAL_SHELL_WIDTH_CLASS,
} from './ExperimentalSideQuestsPanel'

type ShowTellQuadrantModalProps = {
  open: boolean
  onClose: () => void
}

/**
 * Centered “Side quests” (show & tell) quadrant: same content as the home grid’s third tile in
 * reading order (bottom-left) — for quick access from case-study top nav.
 */
export function ShowTellQuadrantModal({ open, onClose }: ShowTellQuadrantModalProps) {
  ...
  return createPortal(
    ...
    >
      <div
        ...
        className={`port-modal-panel cursor-auto overflow-y-auto overflow-x-hidden ${SHOW_TELL_MODAL_SHELL_WIDTH_CLASS}`}
